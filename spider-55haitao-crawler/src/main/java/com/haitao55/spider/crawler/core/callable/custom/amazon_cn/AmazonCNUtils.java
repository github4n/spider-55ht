...
public class AmazonCNUtils {

	private static final Logger logger = LoggerFactory.getLogger(Constants.LOGGER_NAME_CRAWLER);
	private static final Logger loggerDebug = LoggerFactory.getLogger(AmazonCNUtils.class);
	private static final String ENDPOINT = "https://webservices.amazon.cn";
	private static final String HOST = "www.amazon.cn";
	private static final String IDTYPE = "ASIN";

	public static RetBody getRetBody(Context context, AWSKey key, long timeout) throws Exception {
		String url = context.getUrl().getValue();// 需要对各种格式（甚至包括中文）的url做一个规整
		String itemId = SpiderStringUtil.getAmazonItemId(url);
		if(itemId == null)
			itemId = AmazonCNMultiSKUVersion.getAmazonCN_ItemId(url);
		if(StringUtils.isNotBlank(itemId) && StringUtils.contains(itemId, "/ref")){
			itemId = StringUtils.substringBefore(itemId, "/ref");// 临时发现的一个临时的处理
		}
		url = String.format(AmazonCNMultiSKUVersion.ITEM_URL_TMT, itemId);
		
		RetBody rebody = new RetBody();
		String parentAsin = StringUtils.EMPTY;

		try {
			AwsApaRequester requester = new AwsApaRequesterImpl(ENDPOINT, key.getAccessKeyId(), key.getSecretKey(),
					key.getAssociateTag());
			ItemLookupRequest lookup = new ItemLookupRequest();
			lookup.setIdType(IDTYPE);
			lookup.getItemId().add(itemId);
			addReturnKeys(lookup);
			
			logger.info("AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {}", key.getAccessKeyId(),
					key.getSecretKey(), key.getAssociateTag(), itemId);
			
			long start = System.currentTimeMillis();
			Response<ItemLookupResponse> itemLookupResponse = requester.itemLookupAsync(lookup);
			long end = System.currentTimeMillis();
			long interval = (end - start) / 1000;
			loggerDebug.info("call AmazonCNUtils.itemLookupAsync method debug, url:{}, spent-time:{}", url, interval);
				...
				if (items != null) {
					...
					if (itemList != null && itemList.size() > 0) {
						...
						if (item != null) {
							...
							if (offers != null) {
								List<Offer> offerList = offers.getOffer();// 对于有些商品，这里会获取不到商品数据，offerList为空
								if(offerList == null || offerList.isEmpty()){
									loggerDebug.info("call AmazonCNUtils.itemLookupAsync method debug, offerList is empty, url:{}", url);
								}
								if (offerList != null && offerList.size() > 0) {
									...
									if (offer != null) {
										...
										if (salePrice != null) {
											...
											if (StringUtils.isNotBlank(formatSalePrice)) {
												String formatSale = StringUtils.replace(StringUtils.replace(
														formatSalePrice, AmazonCNPage.JPY_SYMBOL, ""), ",", "");
										...
										if (origPrice != null) {
											...
											if (StringUtils.isNotBlank(formatOrigPrice)) {
												String formatOrig = StringUtils.replace(StringUtils.replace(
														formatOrigPrice, AmazonCNPage.JPY_SYMBOL, ""), ",", "");
										...
										rebody.setPrice(
												new com.haitao55.spider.common.gson.bean.Price(orig, save, sale, unit));
									...
									// throw new
									// ParseException(CrawlerExceptionCode.OFFLINE,
									// context.getUrl().toString()+" is
									// offline...");
								}
							}
							...
							if (attr != null) {
								...
								String s_gender = "all";
								if (StringUtils.contains(department, "男")) {
									s_gender = "men";
								} else if (StringUtils.contains(department, "女")) {
									s_gender = "women";
								}
									...
									if (imageSetList != null && imageSetList.size() > 0) {
										...
										for (ImageSet imageSet : imageSetList) {
											...
											if (StringUtils.containsIgnoreCase(category, "swatch")) {
												...
												context.getUrl().getImages().put(System.currentTimeMillis() + "",
														swatchList);
							...
							// assemble sku
							// 11111.assemble l_style_list
							List<LStyleList> l_style_list = Lists.newArrayList();
							LStyleList lStyleList = new LStyleList();
							String colorValue = "default";
							if(StringUtils.isNotBlank(item.getItemAttributes().getColor())){
								colorValue = item.getItemAttributes().getColor();
							}
							...
							// 22222.assemble l_selection_list
							List<LSelectionList> l_selection_list = Lists.newArrayList();
							
							Stock stock = rebody.getStock();
							com.haitao55.spider.common.gson.bean.Price price = rebody.getPrice();
							LSelectionList lselectlist = new LSelectionList();
							lselectlist.setGoods_id("");
							lselectlist.setOrig_price(price.getOrig());
							lselectlist.setSale_price(price.getSale());
							lselectlist.setPrice_unit(price.getUnit());
							lselectlist.setStock_status(stock.getStatus());
							lselectlist.setStock_number(stock.getNumber());
							lselectlist.setStyle_id(colorValue);
							
							List<Selection> selections = new ArrayList<Selection>();
							String sizeValue = item.getItemAttributes().getClothingSize();
						...
						throw new ParseException(CrawlerExceptionCode.OFFLINE,
								context.getUrl().toString() + " is offline...");
					}
				}
			}
		} catch (ParseException e1) {
			e1.printStackTrace();
			logger.error("AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {},ParseException error msg {}",
					key.getAccessKeyId(), key.getSecretKey(), key.getAssociateTag(), itemId, e1.getMessage());
			throw e1;
		} catch (ExecutionException | InterruptedException e2) {
			e2.printStackTrace();
			logger.error(
					"AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {},ExecutionException | InterruptedException error msg {}",
					key.getAccessKeyId(), key.getSecretKey(), key.getAssociateTag(), itemId, e2.getMessage());
			throw e2;
		} catch (TimeoutException e3) {
			e3.printStackTrace();
			logger.error("AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {},TimeoutException error msg {}",
					key.getAccessKeyId(), key.getSecretKey(), key.getAssociateTag(), itemId, e3.getMessage());
			throw e3;
		}
	...
	public static RetBody getRebody(ParentResult parentResult, AWSKey key, long timeout) throws Exception {
		...
		try {
			...
			addReturnKeys(lookup);
			logger.info("AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {}", key.getAccessKeyId(),
					key.getSecretKey(), key.getAssociateTag(), parentResult.getItemId());
			Response<ItemLookupResponse> itemLookupResponse = requester.itemLookupAsync(lookup);
			ItemLookupResponse response = itemLookupResponse.get(timeout, TimeUnit.MILLISECONDS);
			List<Items> itemsList = response.getItems();
			if (itemsList != null) {
									...
									for (Item varItem : varItemList) {
											...
											if (offerList != null && offerList.size() > 0) {
												...
												if (offer != null) {
													...
													if (salePrice != null) {
														...
														String formatSalePrice = salePrice.getFormattedPrice();
														if (StringUtils.isNotBlank(formatSalePrice)) {
															String formatSale = StringUtils
																	.replace(
																			StringUtils.replace(formatSalePrice,
																					AmazonCNPage.JPY_SYMBOL, ""),
																			",", "");
															sale = Float.valueOf(formatSale);
														}
													}
													...
													if (origPrice != null) {
														...
														String formatOrigPrice = origPrice.getFormattedPrice();
														if (StringUtils.isNotBlank(formatOrigPrice)) {
															String formatOrig = StringUtils
																	.replace(
																			StringUtils.replace(formatOrigPrice,
																					AmazonCNPage.JPY_SYMBOL, ""),
																			",", "");
															orig = Float.valueOf(formatOrig);
														}
										...
										if (varItem.getMediumImage() != null) {
											...
											if (StringUtils.isNotBlank(swatchUrl)) {
												...
												parentResult.getContext().getUrl().getImages()
														.put(System.currentTimeMillis() + "", swatchList);
											}
										}
												...
												if (imageSetList != null && imageSetList.size() > 0) {
													for (ImageSet imageSet : imageSetList) {
														...
														if (StringUtils.containsIgnoreCase(category, "variant")) {
															imageList.add(new Image(imageSet.getLargeImage().getURL()));
														} else if (StringUtils.containsIgnoreCase(category,
																"primary")) {
															imageList.add(0,
																	new Image(imageSet.getLargeImage().getURL()));
														}
													}
												}
											}
										...
										if (vatrr != null) {
											...
											for (VariationAttribute va : vas) {
												if (StringUtils.equals(va.getName(), majorDimension)) {
													...
													lStyleList
															.setStyle_switch_img((swatchUrl == null ? "" : swatchUrl));
										...
										l_selection_list.add(lselectlist);
										if (StringUtils.isNotBlank(lStyleList.getStyle_id())
												&& styles.get(lStyleList.getStyle_id()) == null
												&& imageList.size() > 0) {
											l_style_list.add(lStyleList);
			...
			e1.printStackTrace();
			logger.error("AccessKeyId {}, SecretKey {}, AssociateTag {},ItemId {},ParseException error msg {}",
					key.getAccessKeyId(), key.getSecretKey(), key.getAssociateTag(), parentResult.getItemId(),
					e1.getMessage());
			throw e1;
		} catch (ExecutionException | InterruptedException e2) {
			e2.printStackTrace();
