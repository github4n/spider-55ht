 ...
 *
 */
public class Zappos extends AbstractSelect{

	private static final Logger logger = LoggerFactory.getLogger(Constants.LOGGER_NAME_CRAWLER);
	private static final String domain = "www.zappos.com";
			...
			if (StringUtils.isNotBlank(content)) {
					...
					if (stockArr != null) {
						for (int i = 0; i < stockArr.size(); i++) {
							...
							if(price != null && price.containsKey("nowInt")){
								...
								if(sale_price == 0){
									logger.error("Error while crawling zappos saleprice,url {}",context.getUrl().toString());
									stock_status = 0;
								}
							}
							...
							if(orig_price > 0){
								...
								lselectlist.setOrig_price(sale_price);
							}
				...
				if (StringUtils.isNotBlank(colorId)) {
					...
					if(orig > 0){
						...
						rebody.setPrice(new Price(sale, 0, sale, unit));
					}
				...
				Elements es = document.select("div.description > div > ul > li");
				...
				es = document.select("div.description > div > ul > li.measurements > ul > li");
				if (es != null && es.size() > 0) {
					for (Element e : es) {
	...
	public Zappos put(JSONObject obj, String key, String value) {
		obj.put(key, value);
		return this;
	}

	public Zappos put(JSONObject obj, String key, long value) {
		obj.put(key, value);
		return this;
	}

	public Zappos put(JSONObject obj, String key, float value) {
		obj.put(key, value);
		return this;
	}
