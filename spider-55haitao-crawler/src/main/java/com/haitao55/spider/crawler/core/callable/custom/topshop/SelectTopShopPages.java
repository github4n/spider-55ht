...
* Company: 55海淘
* @author zhaoxl 
* @date 2017年1月11日 下午2:21:36
* @version 1.0
 */
public class SelectTopShopPages extends AbstractSelectUrls {
	private static final Logger logger = LoggerFactory.getLogger(Constants.LOGGER_NAME_CRAWLER);
	
	private static final String page_url_preffix = "http://www.topshop.com/webapp/wcs/stores/servlet/CatalogNavigationAjaxSearchResultCmd?catalogId=()&langId=-1&storeId=[]&dimSelected=";
	...
	public void invoke(Context context) throws Exception {
		...
		String templateUrl_temp = this.selectTemplateUrl(context);
...
//		}
		...
		//page页　url封装，　特殊处理
		String dimSelected = StringUtils.substringAfter(templateUrl_temp, "dimSelected=");
		
		String content = this.getInputString(context);
		//截取需要到　catalogId　storeId
		String data = StringUtils.substringBetween(content, "Arcadia.Page = Arcadia.Page || {};", ";");
		data = StringUtils.substringAfter(data, "Arcadia.Page = ");
		JSONObject jsonObject = JSONObject.parseObject(data);
		String catalogId = jsonObject.getString("catalogId");
		String storeId = jsonObject.getString("storeId");
		//改造　page_url_preffix
		String templateUrl = StringUtils.replacePattern(page_url_preffix, "\\(\\)", catalogId);
		templateUrl = StringUtils.replacePattern(templateUrl, "\\[\\]", storeId);
		templateUrl = templateUrl.concat(dimSelected);
		if (StringUtils.isNotBlank(templateUrl)) {
			templateUrl = templateUrl.replaceAll(replaceRegex, replaceFormat);
		}
