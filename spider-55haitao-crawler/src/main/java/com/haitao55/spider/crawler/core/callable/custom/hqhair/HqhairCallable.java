...
class HqhairCallable implements Callable<HqhairSkuBean> {
	private static final Logger logger = LoggerFactory.getLogger(Constants.LOGGER_NAME_CRAWLER);
	private static final String imgprefix = "http://s4.thcdn.com/";
	private Url skuUrl;

	public HqhairCallable(Url skuUrl) {
		this.skuUrl = skuUrl;
	}

	@Override
	public HqhairSkuBean call() {
		HqhairSkuBean skuBean = new HqhairSkuBean();
		skuBean.setUrl(skuUrl.getValue());
		try {
			// String content = HttpUtils.get(skuUrl, HttpUtils.DEFAULT_TIMEOUT,
			...
			if (StringUtils.isBlank(replace(origPrice)) && StringUtils.isBlank(replace(salePrice))) {
				logger.error("Error while crawling skinstore price ,url {}", skuUrl);
				return null;
			}
			...
			if (null != imageJsonArray && imageJsonArray.size() > 0) {
				for (int i = 0; i < imageJsonArray.size(); i++) {
					...
					if ("carousel".equals(map.get("type"))) {
						images.add(new Image(imgprefix.concat(String.valueOf(map.get("name")))));
					}
				}
			}
			skuBean.setImageUrl(images);
			...
			logger.error("skinstore  sku analysis error", e1);
			return null;
		}
		return skuBean;
	}
