...
* @version 1.0
 */
public class NauticaHandler {
	private int threads = 3;
	private ExecutorService executorService;
	public NauticaHandler(){
		executorService = Executors.newFixedThreadPool(threads); 
	}
	public void process(List<String> requestUrls, JSONArray skuJSONArray, Context context) {
		...
		for (String requestUrl : requestUrls) {
			calls.add(new NauticaCallable(requestUrl,context));
		}
		try {
