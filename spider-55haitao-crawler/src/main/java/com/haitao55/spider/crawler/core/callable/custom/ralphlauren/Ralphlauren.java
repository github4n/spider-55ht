...
/**
 * @Description: 拉尔夫劳伦网站详情页抓取类
 * @author: zhoushuo
 * @date: 2016年11月9日 下午4:53:34
 */
public class Ralphlauren extends AbstractSelect {
	...
	private static final String CSS_OFFLINE = "div#oosMessage p";
...
//						"Error while fetching title with url " + context.getCurrentUrl() + " from ralphlauren.");
			...
			List<RalphlaurenSku> skuList = this.getSkuList(content, context, document, switch_imgs, colorImgs.keySet());
			...
			context.getUrl().getImages().put(UUID.randomUUID().toString(), switchImgs);
			...
			/**
			 * 后来在页面源代码中发现，许多页面可以直接取到性别， 但是一来并非所有的页面都能用这种办法获取(如Home类别中的Gift For
			 * Her这种办法就无法取到) 二来因为程序已经通过预发布环境测试，为了减少程序的改动，避免造成不必要的错误
			...
			if(CollectionUtils.isEmpty(skuList)) {
				/**
				 * 因为sku是通过页面源码javascript中截取，如果页面显示的商品库存全部为零，将无法获取其sku
				 * 刚好商品库存全部为零的商品会被SE过滤，而且经过测试发现数量不多，故而未做其他处理，直接丢弃
				 */
			...
			for (RalphlaurenSku sku : skuList) {
				...
				if (!checkStyleIdExist(sku, l_style_list)) {
					...
					context.getUrl().getImages().put(sku.getSkuId() + "", styleImgs);
			...
			if (colorIds.contains(colorId)) {
				...
				try {
					if (StringUtils.isNotBlank(inventory)) {
						inventory = inventory.replace("'", "");// 个别商品库存带单引号，尽可能防止NumberFormatException
