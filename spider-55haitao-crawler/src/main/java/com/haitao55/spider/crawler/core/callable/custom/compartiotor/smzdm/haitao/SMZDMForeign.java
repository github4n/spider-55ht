package com.haitao55.spider.crawler.core.callable.custom.compartiotor.smzdm.haitao;
...
* @version 1.0
 */
public class SMZDMForeign extends AbstractSelect{
	private static final String NEW = "http://haitao.smzdm.com/p";
	private static final String domain = "haitao.smzdm.com";
	public void invoke(Context context) throws ClientProtocolException, HttpException, IOException {
		...
		if(StringUtils.isBlank(buy_url)){
			throw new com.haitao55.spider.crawler.exception.ParseException(CrawlerExceptionCode.OFFLINE,"haitao.smzdm.com itemUrl:"+context.getUrl().toString()+" is exception..");
		}
...
//		end_time = StringUtils.substringBetween(content, "促销截止至", "。");
		...
		if(StringUtils.containsIgnoreCase(parent_url, NEW)){
			tag = "最新";
		}else{
			tag = "热门";
		}
