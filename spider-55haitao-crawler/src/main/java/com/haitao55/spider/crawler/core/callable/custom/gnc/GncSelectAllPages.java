...
public class GncSelectAllPages extends SelectUrls{
	...
	public void invoke(Context context) throws Exception {
		...
		String pageTotal = StringUtils.substringBetween(content, "searchResultsCount\":", ",");

		if(StringUtils.isNotBlank(pageTotal) 
				&& !"0".equals(pageTotal)){
			Double page  = Double.valueOf(pageTotal) / 64;
			int pageNumber =  (int)Math.ceil(page);
			for(int i = 0 ;i < pageNumber; i++){
				String attr = "?sz=64&start="+i*64+"";
				newUrlValues.add(url+attr);
			}
		}
		Set<Url> newUrls = this.buildNewUrls(newUrlValues, context, type, grade);
		context.getUrl().getNewUrls().addAll(newUrls);
		logger.info("GnuSelectAllPages list url:{} ,get items :{}", context.getUrl().getValue(), newUrlValues.size());
	...
	public static void main(String[] args) throws ClientProtocolException, HttpException, IOException {
		...
		String html = Crawler.create().timeOut(30000).url("http://www.gnc.com/").method(HttpMethod.GET.getValue())
				.proxy(true).proxyAddress("114.55.10.105").proxyPort(3128).resultAsString();
		Document  doc = Jsoup.parse(html);
		int count = 0;
		Elements es = doc.select(".menu-category li .level-2 a");
		for(Element e : es){
			count ++;
