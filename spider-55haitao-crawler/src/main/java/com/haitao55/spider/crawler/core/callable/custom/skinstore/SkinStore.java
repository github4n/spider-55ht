...
public class SkinStore extends AbstractSelect {
	private static String requestUrl = "http://www.skinstore.com/variations.json?productId=";
	...
	private static String domain = "www.skinstore.com";
	...
	public void invoke(Context context) {
		...
		String title = StringEscapeUtils
				.unescapeHtml4(StringUtils.substringBetween(content, "productTitle: \"", "\","));
		...
		Map<String, SkinStoreSkuBean> skuResult = new SkinStoreHandler().process(skuUrlsList);
		// sku
		List<LSelectionList> l_selection_list = new ArrayList<LSelectionList>();
		List<LStyleList> l_style_list = new ArrayList<LStyleList>();
		if (null != skuResult && skuResult.size() > 0) {
			for (Entry<String, SkinStoreSkuBean> entry : skuResult.entrySet()) {
				String skuId = entry.getKey();
				SkinStoreSkuBean skuBean = entry.getValue();
				// selectlist
				...
				if (!skuFlag) {
					rebody.setPrice(new Price(Float.valueOf(skuBean.getOrig()), Integer.parseInt(skuBean.getSave()),
							Float.valueOf(skuBean.getSale()), skuBean.getUnit()));
	...
	private void skuUrlsPackage(Document doc, String productId, List<Url> skuUrlsList, Context context) {
		...
		// [{1={5=variation1, 10499=option1}},{1={5=variation1, 10501=option1}},
		// {2={6=variation2, 2539=option2}}, {3={7=variation3, 5875=option3}}]
		if (null != skuParam && skuParam.size() > 0) {
			...
			// [{1={5=variation1, 10499=option1}},{1={5=variation1,
			// 10501=option1}}, {2={6=variation2, 2539=option2}},
			// {3={7=variation3, 5875=option3}}]
			if (null != form1 && form1.size() > 0) {
				// String
				// sku_url=requestUrl.concat(productId).concat(requestParam);
