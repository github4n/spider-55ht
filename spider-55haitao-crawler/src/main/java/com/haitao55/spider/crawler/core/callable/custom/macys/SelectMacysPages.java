 ...
 * @see SelectUrls
 */
public class SelectMacysPages extends AbstractSelectUrls {
	...
	public void invoke(Context context) throws Exception {
		...
		for (int i = Integer.parseInt(startIndex); i <= totalPagesInt; i++) {
			if(split.length!=2){
				logger.error("macys grade 1 link url",templateUrl);
			}else{
				String pageUrl = split[0].concat(PAGEINDEX).concat(String.valueOf(i)).concat("?").concat(split[1]);;
