...
public class TBMerchantSelectUrls extends SelectUrls{
	...
	public void invoke(Context context) throws Exception {
		...
		if(StringUtils.isNotBlank(content)){
			...
			for(int i = 0; i < jsonArray.size(); i++){
				...
				String url = urlJsonObject.getString("storeLink");
				if(StringUtils.isNotBlank(url)){
					newUrlValues.add("https:"+url);
				}
			}
		}
		Set<Url> newUrls = this.buildNewUrls(newUrlValues, context, type, grade);
		context.getUrl().getNewUrls().addAll(newUrls);
		logger.info("TBMerchantSelectUrls item url:{} ,get items :{}", context.getUrl().getValue(), newUrlValues.size());
	}
