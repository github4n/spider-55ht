...
* @version 1.0
 */
public class SelectDealmoonPages extends AbstractSelectUrls {
	...
	public void invoke(Context context) throws Exception {
		...
		if(StringUtils.containsIgnoreCase(url, HOT)){
			templateUrl = url + "/{}";
		}
		else if(StringUtils.containsIgnoreCase(url, NEW)){
			templateUrl = url + "&p={}";
		}
		else if(StringUtils.containsIgnoreCase(url, TOP) && !StringUtils.containsIgnoreCase(url, NEW)){
			templateUrl = url + "&p={}";
		}
