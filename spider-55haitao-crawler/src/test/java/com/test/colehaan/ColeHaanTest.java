 ...
 * @Description:
 * @author: zhoushuo
 * @date: 2016年12月20日 下午3:57:47
 */
public class ColeHaanTest extends AbstractSelect {
...
//					String url = e.absUrl("href");
//					url = DetailUrlCleaningTool.getInstance().cleanDetailUrl(url);
//					String color_content = HttpUtils.get(url, 30000, 1, null);
//					Document color_document = Jsoup.parse(color_content, url);
//					String current_color = setColorList(color_document, colors);
//					String current_color_productID = StringUtils.EMPTY;
//					Elements current_color_productids = color_document.getElementsByAttributeValue("itemprop", "productID");
//					if (CollectionUtils.isNotEmpty(current_color_productids)) {
//						current_color_productID = StringUtils.trimToEmpty(current_color_productids.get(0).text());
//					}
//					if (StringUtils.isBlank(current_color_productID)) {
...
//						return;
//					}
//					setSkus(color_document, currentUrl, logger, current_color, current_color_productID, skus);
//				}
//			}
			

			for (ColorImgs co : colors) {
				System.out.println(co.getColor());
				// System.out.println(co.getSwitch_img());
				// for(String str : co.getImgs()){
				// System.out.println("当前颜色："+str);
				// }
			}
...
//					context.getUrl().getImages().put(sku.getSkuId(), CrawlerUtils.convertToImageList(colorImgs.getImgs()));
			...
			LImageList image_list = CrawlerUtils.getImageList(colors.get(0).getImgs());
...
//	private void setSkusByColor(Document document, String currentUrl, Logger logger, String color, String productID,
//			List<ColeHaanSku> skus) {
//		// 有color、size和width的情况
//		Elements ewidth = document.select("select#va-width>option:not(:first-child)");
//		Elements esize = document.select("select#va-size>option:not(:first-child)");
//		if (CollectionUtils.isNotEmpty(ewidth) && CollectionUtils.isNotEmpty(esize)) {
//			for (Element ew : ewidth) {
//				String width = StringUtils.trim(ew.ownText().replace("Sold Out", ""));
//				String currentColor = color;
//				String color_param = currentColor.replace(" ", "+");
//				for (Element es : esize) {
//					ColeHaanSku sku = new ColeHaanSku();
//					String size = StringUtils.trim(es.ownText().replace("Sold Out", ""));
//					sku.setColor(currentColor);
//					sku.setSize(size);
//					sku.setWidth(width);
//					sku.setSkuId((currentColor + size + width).replace(" ", ""));
//					String url_sku = "http://www.colehaan.com/on/demandware.store/Sites-ColeHaan_US-Site/en_US/Product-Variation?pid="
//							+ productID + "&dwvar_" + productID + "_size=" + size + "&dwvar_" + productID + "_width="
...
//					String inventory_size = "";
//					String inventory_width = "";
//					if (CollectionUtils.isNotEmpty(e_size)) {
//						for (Element e : e_size) {
//							if (e.text().contains(size)) {
//								inventory_size = e.text();
//								break;
//							}
//						}
//					}
//					Elements e_width = doc.select("select#va-width>option:not(:first-child)");
//					if (CollectionUtils.isNotEmpty(e_width)) {
//						for (Element e : e_width) {
//							if (e.text().contains(width)) {
//								inventory_width = e.text();
//								break;
//							}
//						}
//					}
//					if (StringUtils.isBlank(inventory_size) || StringUtils.isBlank(inventory_width)) {
//						sku.setStatus(0);
//						sku.setInventory(0);
//					}
//					if (inventory_size.contains("Sold Out") || inventory_width.contains("Sold Out")) {
//						sku.setStatus(0);
//						sku.setInventory(0);
//					} else {
//						sku.setStatus(1);
...
//					}
//
//					Elements esaleprice = doc.select("div.product-price span.price-sales");
//					String salePriceStr = "";
//					if (CollectionUtils.isNotEmpty(esaleprice)) {
//						salePriceStr = esaleprice.get(0).text();
//					}
//					sku.setUnit(CrawlerUtils.getUnit(salePriceStr, currentUrl, logger));
//					sku.setSale_price(CrawlerUtils.getPrice(salePriceStr, currentUrl, logger));
//					skus.add(sku);
//				}
//			}
//		}
...
//		Elements esize = document.select("select#va-size>option:not(:first-child)");
	...
	public static void main(String[] args) throws Exception {
		ColeHaanTest cht = new ColeHaanTest();
		Context context = new Context();
		context.setCurrentUrl("http://www.colehaan.com/eliza-grand-pump-%2885mm%29-molten-lava-suede/W05515.html");
		context.setCurrentUrl(
				"http://www.colehaan.com/tali-grand-lace-wedge-%2840mm%29-sahara-snake-print/W00760.html");
		context.setCurrentUrl("http://www.colehaan.com/tali-grand-lace-wedge-%2840mm%29-black/W00030.html");
		context.setCurrentUrl("http://www.colehaan.com/women%27s-pinch-leather-weekender-black-white-leather/W03475.html?dwvar_W03475_color=Black-white%20Leather&dwvar_W03475_width=B#cgid=womens_shoes_loafersdrivers&start=9");
		context.setCurrentUrl("http://www.colehaan.com/women%27s-pinch-leather-weekender-soft-gold-metallic/D44621.html?dwvar_D44621_color=Soft%20Gold%20Metallic");
		//只有Color
		context.setCurrentUrl("http://www.colehaan.com/genevieve-weave-large-triangle-tote-sequoia/U01962.html?dwvar_U01962_color=Sequoia&dwvar_U01962_width=B#cgid=womens_handbags&start=4");
		//color size
//		context.setCurrentUrl("http://www.colehaan.com/flat-strap-suede-belt-black/U01577.html?dwvar_U01577_color=Black&dwvar_U01577_width=B#cgid=womens_accessories_belts&start=2");
		context.setCurrentUrl("http://www.colehaan.com/women%27s-lockridge-waterproof-lace-boot-burnt-chili-waterproof/W05466.html");
		cht.invoke(context);
	}
}
