...
public final class EncodingSniffer {
	...
	private static final byte[][] COMMENT_START = new byte[][] { new byte[] { '<' }, new byte[] { '!' },
			new byte[] { '-' }, new byte[] { '-' } };
	 ...
	 * tag.
	 */
	private static final byte[][] META_START = new byte[][] { new byte[] { '<' }, new byte[] { 'm', 'M' },
	 ...
	 * content.
	 */
	private static final byte[][] OTHER_START = new byte[][] { new byte[] { '<' }, new byte[] { '!', '/', '?' } };

	/**
	 * Sequence(s) of bytes indicating the beginning of a charset specification.
	 */
	private static final byte[][] CHARSET_START = new byte[][] { new byte[] { 'c', 'C' }, new byte[] { 'h', 'H' },
			new byte[] { 'a', 'A' }, new byte[] { 'r', 'R' }, new byte[] { 's', 'S' }, new byte[] { 'e', 'E' },
			new byte[] { 't', 'T' } };
	 ...
	 * looking for <tt>Content-Type</tt> information in the HTTP headers and
	 * <a href="http://en.wikipedia.org/wiki/Byte_Order_Mark">Byte Order
	 * Mark</a> information in the content.
	 * </p>
	 * 
	...
	static String sniffEncodingFromUnicodeBom(final byte[] bytes) {
		...
		if (bytes != null && Objects.deepEquals(markerUTF8, ArrayUtils.subarray(bytes, 0, 3))) {
			encoding = UTF8;
		} else if (bytes != null && Objects.deepEquals(markerUTF16BE, ArrayUtils.subarray(bytes, 0, 2))) {
			encoding = UTF16_BE;
		} else if (bytes != null && Objects.deepEquals(markerUTF16LE, ArrayUtils.subarray(bytes, 0, 2))) {
			encoding = UTF16_LE;
		}
		return encoding;
	}
	...
	static String sniffEncodingFromXmlDeclaration(final byte[] bytes) {
		...
		if (Objects.deepEquals(declarationPrefix, ArrayUtils.subarray(bytes, 0, declarationPrefix.length))) {
